...
class PostControlTest {
    ...
    public void whenSavePost() throws Exception {
        this.mockMvc.perform(post("/save")
                .param("name", "BMW X5 M")
                .param("description", "Куплю BMW X5 M дорого"))
                .andDo(print())
                .andExpect(status().is3xxRedirection());
        ArgumentCaptor<Post> argument = ArgumentCaptor.forClass(Post.class);
        verify(posts).create(argument.capture(), eq("user"));
        assertThat(argument.getValue().getName(), is("BMW X5 M"));
        assertThat(argument.getValue().getDescription(), is("Куплю BMW X5 M дорого"));
    }
}